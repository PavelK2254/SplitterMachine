let MongoClient;_fb9‍.x([["init",()=>init],["addNewOrdersToDB",()=>addNewOrdersToDB],["getOrderByBarcode",()=>getOrderByBarcode]]);_fb9‍.w("mongodb",[["MongoClient",["MongoClient"],function(v){MongoClient=v}]]);const collectionName = "ordersTest";
...
var availableCollections = [];
    ...
    return new Promise(function(resolve, reject) {
        MongoClient.connect(url, function(err, db) {
            if (err) reject(err);
            var dbo = db.db(dbName);
            var orderBoject = dbo.collection(collectionName).findOneAndUpdate({ barcodeNum: +barcode, status: 0 }, { $set: { status: 1 } }, function(err, result) {
                if (err) {
                    reject(err);
                } else {
